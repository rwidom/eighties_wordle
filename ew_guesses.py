...
from ew_display import guess_display, keyboard_display, numpad_display
...
class all_the_guesses:
    ...
    def _try_again(self, word, instruction):
        ...
        if remaining_goofs <= 0:
            bad_guess = (
                word[: self.word_length].lower().strip().rjust(self.word_length, "*")
            )
    ...
    def take_a_guess(self):
        ...
        Side effects:
            ...
            it is added to the list of goofs, we ask the user to try again, and we check
            again.
            - If the word *IS* valid, we add it to the list of guesses in the last
            position.
        """
        ...
        if (w.isnumeric() and len(w) <= 2) or w == "*":
            ...
            for (i, guess) in enumerate(self.guesses):
                if guess in self.all_words and max(
                    [g == a for (g, a) in zip(guess, self.answer)]
                ):
                    valid_turns += [i + 1]
            ...
            if len(valid_turns) == 0:
                self.print_header()
                print(
                    "Sorry, you need a turn with at least one thing in the right",
                    "position to get a hint.",
                )
                self.take_a_guess()
            elif w == "*":
                turn = valid_turns[-1]
                ...
                self.print_header()
                print(
                    "For a hint, please choose from one of these turns -- "
                    + ", ".join([str(t) for t in valid_turns])
                )
                self.take_a_guess()
            # generate the hint list
            if turn > 0:
                word_with_blanks = guess_display(
                    self.guesses[turn - 1], self.answer
    ...
    def print_header(self):
        ...
        if len(self.guesses) == 0:
            ...
            print(
                "I'm thinking of a",
                self.word_length,
                (self.game_type == "words") * "letter word."
                + (self.game_type == "equations") * "digit equation.",
                "Start guessing!",
            )
    ...
    def next_step(self):
        ...
        Won't work at the very beginning of the game, need to call print header and
        take_a_guess
        """
...
class elimination_guesses(all_the_guesses):
    ...
    def __init__(self, game_dictionary, game_length, first_guess=None):
        ...
        super(elimination_guesses, self).__init__(
            game_dictionary=game_dictionary, game_length=game_length, game_type="words"
        )
        # modify for absurdle, re-initializes possible_words, answer, guesses, keyboard
        # with a random guess
    ...
    def get_alphabet_tuple(self, word):
        """
        Returns a 26 length tuple of tuples with the position(s) of each letter
        represented by the position in the tuple
        """
        assert type(word) == str
        alpha_positions = [tuple()] * 26
        for (position, letter) in enumerate(word):
            l_index = string.ascii_uppercase.index(letter)
            alpha_positions[l_index] += tuple([position])
        return tuple(alpha_positions)
    ...
    def get_bucket_id(self, guess, answer):
        ...
        Output: a tuple of 26 values representing the known position(s) of each letter
            ...
            - within the tuple, int values are known position(s), None values are
              unknown positions
        """
        ...
        for i in range(26):
            ...
            # compare positions
                ...
                bucket_id[i] = tuple(known_positions) + tuple(
                    [None] * unknown_positions
                )
        return tuple(bucket_id)

    def prune_word_list(self, new_guess=None):
        """
        Modifies the all possible words list to take the largest group with a single
        status, relative to the given guess. If no guess is specified, will take a
        random word.
        """
        ...
        for word in self.possible_words:
            # tuple of statuses: 0 = not in word; 1 = in word, wrong place; 2 = in the
            # right place
            ...
            if statuses in self.buckets:
                self.buckets[statuses] += [word]
            else:
                self.buckets[statuses] = [word]
        # choose the biggest list
        most_common_pattern = sorted(
            self.buckets, key=self.get_bucket_length, reverse=True
        )[0]
...
if __name__ == "__main__":
    ...
    for sg in sample_games:
        ...
        for turn in sg:
            ...
            print(
                "Pruned list of possible words based on",
                turn,
                "has",
                len(g.possible_words),
                "words, starting with...",
            )
            ...
            print(
                ...
                g.answer,
                "(",
                g.possible_words.index(g.answer),
                ")",
            )
