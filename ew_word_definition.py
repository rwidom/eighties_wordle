...
def get_first_definition(word):
    ...
    if type(api_call) == str:
        ...
        # verbs generally have two definitions: one for transitive and one for
        # intransitive
        ...
        if "sseq" not in step1:
            ...
            # we take the first sense, which is itself a list, which can represent a
            # complex tree
            step2 = step1["sseq"][0]
            ...
            if "dt" in step3:  # definition text, also a list of lists
                ...
                return (
                    "??? definition for the first sense of the meaning has no text ???"
                )
...
def internet(host="8.8.8.8", port=53, timeout=3):
    ...
    """
    # lifted shamelessly from https://stackoverflow.com/questions/3764291
    try:
...
if __name__ == "__main__":
    ...
    # assume that the game dictionary only has real words that at least have aliases in
    # dictionary.com
    sample_words = [
        "BLUMED",
        "LIERS",
