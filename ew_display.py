...
class guess_display:
    ...
    def __init__(self, guess, answer):
        """
        Input: guess and answer are both valid words. Any data quality issues (case,
            length, alphas only) happens outside of the printer object.
    ...
    def get_string_dict(self, input_string=False):
        """
        Returns a dict with the key of each letter, and values of a list of positions
        for that letter
        ...
        if self.guess == self.answer:
            ...
            for (letter, locations) in guess_dict.items():
                ...
                if letter in answer_dict:
                    ...
                    # what are the locations we guessed, excluding the ones that were
                    # correct?
                    ...
                    no = locations
                letter_tuple_list += [
                    (location, letter_display(letter, "OK")) for location in ok
                ]
                letter_tuple_list += [
                    (location, letter_display(letter, "MOVE")) for location in move
                ]
                letter_tuple_list += [
                    (location, letter_display(letter, "NO")) for location in no
                ]
    ...
    def __str__(self):
        ...
        """
        return " ".join([str(letter) for letter in self.get_letter_statuses()])
    ...
    def display(self):
        ...
        """
        for letter in self.get_letter_statuses():
            letter.display()
        print()
    ...
    def get_correct_letters_plus_blanks(self):
        ...
        """
        return "".join(
            [g * (g == a) + "_" * (g != a) for (g, a) in zip(self.guess, self.answer)]
        )
...
class keyboard_display:
    ...
    def __init__(self, answer, guesses=[]):
        ...
        Input: at least an answer, in case there aren't any guesses yet, but ideally an
            answer and a list of guesses that are all valid words. Any data quality
            issues (case, length, alphas only) happens outside of the printer object.
        Output: successfully initializes the object
        TO DO:
            write nicer getter and setters
        """
        ...
        # standards for the whole class, written here so they can be easily overwritten
        # in the subclass
        self.keys = "QWERTYUIOP ASDFGHJKL ZXCVBNM "
        self.vertical_border = "\u2551"
        if p.IS_IPHONE is False:
            self.top_border = "\u2554" + ("\u2550" * 40) + "\u2557"
    ...
    def get_letter_locations(self):
        ...
        Output: A dictionary where the keys are every letter that has been guessed and
        the values are the positions where the letter has appeared
        """
    ...
    def get_letter_displays(self):
        ...
        for letter in self.keys:
            if letter in guess_locations:
                ...
                # guessed in the right position, the intersection of these sets is
                # truthy / not empty
                elif guess_locations[letter].intersection(
                    set([i for i, l in enumerate(self.answer) if l == letter])
                ):
                    letter_displays += [letter_display(letter, "OK")]
                # guessed and it's in the answer, but never guessed in the right
                # position
                else:
                    letter_displays += [letter_display(letter, "MOVE")]
            # not guessed yet
            else:  # make sure that \n is read as one char, not in any of the guesses
    ...
    def __str__(self):
        ...
        """
        lines = [self.top_border] + [""] * len(self.line_padding) + [self.bottom_border]
        line = 1
        line_start = True
        for k in self.keyboard:
            ...
            if line_start:
                ...
                line_start = False
            # line break in the keyboard string, print the end of the line and reset
            # line_start
            if k.get_letter() == " ":
